...
| 3 | A | ~~**react-bootstrap**, mobile-first~~ **AMENDED 2026-09-13 → Tailwind v4 + shadcn/ui** (Alexander's call, executed pre-#14 while the swap cost ≈1h: the design workflow leans on AI tools that emit Tailwind near-verbatim; the widget-dashboard ambition wants design freedom over Bootstrap's default look; utility-class verbosity falls on Claude, not Alexander. shadcn = copy-in Radix components we own; theme tokens in `src/index.css`) | Original rationale: familiarity beats marginal ergonomics solo; known cost was the default look — which the amendment resolves | Mantine/MUI (richer tables/pickers); staying on react-bootstrap (rejected: translation tax on every AI-generated design) |
