...
| # | Task | Priority | Estimate | Plan |
|---|------|----------|----------|------|
| 19 | 🏺 E7 Pots, targets & labels — the dec 22/23 model: pot TREE with rollups, amount-splits, free labels, monthly targets, payday config (ST3), month-end remainder (ST5), account nicknames. **Needs `/grill-me` before build** (schema + §2a invariants + dec 22/23 constraints). **UI already designed**: canvas options 2a–2c (design project 69558837…) — but only the VIEWS; Session 03 dec 39 parked to this task: pot setup/management flows, the provisional Pots 4th tab, and the month-end "allocate" action. Method (Alexander 2026-09-21): grill first → the grilled spec becomes the Session 04 design requirements. Absorbs the old "Pots & budgeting" backlog row | 🟡 P2 | 3–4d | [session](../notes/product/design-session-02-user-stories.md) |
| 15 | 🛡️ E3 Settings & data rights — settings page: profile, Disconnect Monzo, Export JSON, **Delete+Purge** (instant, typed confirm, Monzo consent revoke). New server endpoints — **`/grill-me` the delete/export spec before build** (dec 9–12). **UI already designed**: canvas options 4a–4e + Session 03's 8a–8g (desktop rail IA, appearance picker, sessions & devices, export/delete journeys past the confirm dialog, sync-now states, About — dec 40–43 in [session 03](../notes/product/design-session-03-supporting-views.md); 4d delete dialog amended per dec 43: will/won't ledger, disconnect-instead de-escalation, typed phrase "delete my account"). ⚠️ **Grill must also decide session POLICY**: 8c designs multi-device sessions with per-device revocation, but the backend has been single-session since #14 (pre-#14 it was multi) — passkeys-per-device (canvas 6f–6h) pulls toward multi-session; either relax the policy or collapse 8c to current-device-only | 🟡 P2 | 1–2d | [session](../notes/product/design-session-01-top-down.md) |
| 17 | 🌐 E5 Edge & deploy — Dockerfile, NUC deploy, Cloudflare Tunnel + Access identity allowlist, one URL everywhere, security headers/CSP/CORS (dec 13–15). Any time after #14; required before daily phone use. Also: point the Monzo dev-portal **Logo URL** at `https://<domain>/icon-192.png` once the domain exists — it's a static asset on the web host, NOT an API endpoint, and it needs a stable public URL (ngrok rotates, so it can't carry the consent-screen logo) | 🟡 P2 | 1–2d | [session](../notes/product/design-session-01-top-down.md) |
...
| 🔁 ~~Reconnect catch-up delta~~ + manual Sync now — **DONE in feature/design-pass (2026-09-20)** | — | — | Both built same-day at Alexander's direction: (1) `TransactionSyncService.backfill` chains `deltaSync` for COMPLETED accounts whose backfill fetched zero windows, so a relink closes the dormant-period gap in the same pass; (2) `POST /api/v1/monzo/sync` = `syncNow(userId)` — per-account delta + one `runFullPass`, the hourly job on demand — surfaced as a "Sync now" button on the Settings connection row (invalidates the whole query cache on success). Webhooks (#5) later shrink the delta to ~0 between manual syncs. Move to Done when the PR merges |
| 🪝 #5 Webhooks | P3 | TBD | [plan](open/webhooks/plan.md) — second trigger into the raw→domain pipeline + near-real-time balance refresh. Package home when built: a dedicated `api/webhook` (or `api/public`) top-level package beside `api/{dev,health,v1}` — endpoints Monzo's SERVERS call directly, with their own security chain (signature verification, no cookies/JWE), so publicness is visible in the package tree, not just the SecurityConfig permit list (Alexander 2026-09-20). **Moved behind frontend epics (Session 01)**; ⚠️ Monzo's servers can't pass Cloudflare Access — needs a deliberate bypass route with its own signature verification (Session 01 dec 17) |
| 🧩 Widget dashboard | P3 | TBD | Customisable pick-and-choose widget summary (Session 01 dec 18 recorded the vision; dashboard v1 ships fixed). Session 02 dec 27: v1 blocks are built widget-shaped — configurability layers on after #19 proves them. Session 03 dec 38/39: frame contract = canvas 7a (five slots; default/skeleton/empty/error — empty is a true zero, error keeps last good figure, never red), layout = 7-ii bento collapsing to the stack under 720px; customize mode (7e) parked — drag-and-drop rejected as touch-idiom, use add-widget sheet + show/hide + reorder list when it comes |
...
| 👻 Ghost-session consistency (@CurrentUserId vs @CurrentUser) | P3 | 0.25d | Seen live 2026-09-20 after a DB wipe: same ghost JWE got 401 from `/me`+`/sync/progress` (@CurrentUser → DB lookup catches the missing row) but 200-with-empty-data from `/accounts`+`/transactions` (@CurrentUserId → no lookup). Harmless (data is user-scoped, `/me` 401 logs the client out) but asymmetric. Superseded in practice by the per-request session-validation row (DECIDED 2026-09-20) — the filter check resolves this asymmetry for free; keep this row only as the test checklist when that lands |
| 🚪 Server-side session validation on every request (drop pure statelessness) | P2 | 0.5–1d | **DECIDED — Alexander 2026-09-20**: for this app's size and security posture, one indexed DB check per request is worth true revocation — logout/delete must kill ACCESS tokens instantly, not at TTL expiry. Design: `JweAuthenticationFilter` validates the JWE crypto as today, then verifies the token's session claim against the sessions table (the refresh-session row #14 already keeps; add a session-id/jti claim to the access token if absent) — revoke the row ⇒ every token for it dies. Prefer this over a separate jti denylist: one mechanism, uses existing single-session infra. Bonus: the filter then also catches ghost sessions (wiped/deleted users) before ANY controller, resolving the @CurrentUserId vs @CurrentUser asymmetry row for free. Perf: PK lookup per authenticated request, negligible at solo scale; can add a short in-memory cache later if ever needed. Slot: before #17 internet exposure; natural neighbour of #15 delete+purge (purge ⇒ sessions gone ⇒ tokens dead instantly) |
| 🃏 Hide £0 active-card checks | P3 | 0.25d | Alexander 2026-09-20: Monzo's £0.00 "Pending" card-auth pings (Google etc.) clutter the transaction list. Option: filter `amount == 0 && PENDING` out of the default view (or collapse behind a toggle). He said "might just leave it" — do only if it keeps annoying |
| 🔐 Auth methods v2 (passwordless-first epic) | P2 | 3d | Alexander 2026-09-18, shaped in discussion. **Phase 1 — COMMITTED (rides the post-Session-02 login polish):** same-browser nonce cookie (B, link-forwarding protection) + email OTP code (D, device choice/freedom) — one email carries button AND short code; same-device clicks, cross-device types; mismatch → informative no-session page. **UI already designed**: canvas options 4f–4h (design project 69558837…). **Phase 2 (headline): passkeys** — enrolment in settings (#15 neighbour), passkey-first login, magic-link/OTP fallback+recovery; passkey ≈ phishing-resistant MFA in one gesture. **Phase 3 (only if still wanted after passkeys): TOTP as optional step-up.** **Passwords: NEVER (Alexander 2026-09-18, final)** — reintroducing them is what would force a mandated-2FA ceremony; passwordless-first is the convention — and note Cloudflare Access is already an outer identity gate that can enforce its own MFA. Absorbs the cross-device-handoff icebox row (D solves it) |
